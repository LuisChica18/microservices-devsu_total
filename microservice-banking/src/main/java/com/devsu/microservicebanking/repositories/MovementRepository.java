...
public interface MovementRepository extends JpaRepository<Movement, Long> {
    ...
    @Query(value = "SELECT DATE_FORMAT(movimientos.fecha, '%Y-%m-%d %H:%i:%s') AS Fecha, persona.nombre AS Cliente, " +
            ...
            "INNER JOIN cuentas_clientes on cuentas_clientes.cuenta_id = cuenta.id " +
            "INNER JOIN cliente ON cuentas_clientes.cliente_id = cliente.id " +
